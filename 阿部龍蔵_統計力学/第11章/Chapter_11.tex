...
\begin{frame}
    ...
    \begin{align}
        e^{-\beta\mathscr{H}}=e^{-\beta\mathscr{H}_0}U(\beta)
    \end{align}
...
\begin{frame}
    ...
    \begin{block}{$T$記号}
        ...
        \begin{align}
            T[C(1),C(2)] = P[C(2),C(1)]\\
            T[a^{(\dagger)}(1),C(2)] = T[C(2),a^{(\dagger)}(1)]
        \end{align}
    \end{block}
    \centering
...
\begin{frame}
    $U^{-1}(u)$を次のように定める
    \begin{align}
        \left(e^{-u\mathscr{H}}\right)^{-1} = \left(e^{-u\mathscr{H}_0}U(u)\right)^{-1} = U^{-1}(u)e^{u\mathscr{H}_0}
    \end{align}
    これを用いて次の表記を定義する
    \setcounter{equation}{23}
    \begin{align}
        U(u,u^{'})=U(u)U^{-1}(u^{'}) = T\exp\left[-\int_{u^{'}}^uH^{'}(u_1)du_1\right]
    \end{align}
    以降, $a_r$に対して
    \begin{itemize}
